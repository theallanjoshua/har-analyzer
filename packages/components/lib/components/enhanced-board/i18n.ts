...
function createAnnouncement(
	...
	const conflictsAnnouncement = conflicts.length > 0 ? `Conflicts with ${conflicts.map(({ data: { componentType, instanceId } }) => `Component type: ${componentType}, Instance ID: ${instanceId}`).join(', ')}.` : '';
